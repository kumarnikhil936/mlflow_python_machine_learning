...
class Trainer():
    def __init__(self, experiment_name, data_path, autolog, save_signature, run_origin="none"):
    ...
    def train(self, max_depth, max_leaf_nodes, model_name, output_path):
        with mlflow.start_run(run_name=self.run_origin) as run:  # NOTE: mlflow CLI ignores run_name
            ...
            plot_file = "plot.png"
            create_plot_file(self.y_test, predictions, plot_file)
...
@click.command()
@click.option("--experiment_name", help="Experiment name.", default="sklearn_wine", type=str)
@click.option("--data_path", help="Data path.", default="wine-quality-white.csv", type=str)
@click.option("--model_name", help="Registered model name.", default="sklearn_wine", type=str)
...
def main(experiment_name, data_path, model_name, max_depth, max_leaf_nodes, output_path, autolog,
         save_signature, run_origin):
    print("Options:")
    ...
    trainer = Trainer(experiment_name, data_path, autolog, save_signature, run_origin)
